...
def run_on_setting(args, **kwargs):
    ...
    kmeans = KMeans(n_clusters=mv_dataset.num_class, n_init=10, random_state=ds_config["seed"])
...
def run():
    ...
    parser.add_argument("--dataset", type=str, default="BBCSport", help="Dataset used for Training.")
    ...
    # run_on_setting(args)
    # 参数搜索
    latent_dim = [8, 16, 32, 64, 256] #
    p = [1, 2, 3, 4, 8, 16] # , 2, 3, 4, 8, 16
    ...
    for latent_dim, p, learning_rate, normalize, batch_size in parameter_iter:
        ...
        print(f"latent_dim={latent_dim}, p={p}, learning_rate={learning_rate}, normalize={normalize}, best_result={best_result}, final_result={final_result}")
        # 将结果记录到文件
        f.write(f"latent_dim={latent_dim}, p={p}, learning_rate={learning_rate}, normalize={normalize}, best_result={best_result}, final_result={final_result}\n")
        f.flush()
    f.close()
